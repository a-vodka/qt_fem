...
namespace elem
{
	...
	int ShapeQuad3D::degeneration(std::vector<int> &ind)
	{
		// Element should be quadratic
		if ((ind[0] == ind[7] && ind[7] == ind[6]) && (ind[8] == ind[11]) && (ind[12] == ind[19] && ind[19] == ind[18]))
		{
			return 1;
		}
		else
	...
	void ShapeQuad3D::shape(double xi, double et, double ze, std::vector<int> &ind, std::vector<double> &n)
	{
		double s0 = 1 + xi;
		double t0 = 1 + et;
		double d0 = 1 + ze;
		...
		double d2 = 1 - ze * ze;
		// Midside nodes
		n[1] = n[3] = n[5] = n[7] = n[8] = n[9] = n[10] = n[11] = n[13] = n[15] = n[17] = n[19] = 0;
		if (ind[1] > 0)
		{
			n[1] = 0.25 * s2 * t1 * d1;
		}
	...
	double ShapeQuad3D::deriv(double xi, double et, double ze, std::vector<int> &ind, std::vector<std::vector<double>> &xy, std::vector<std::vector<double>> &dnxy)
	{
...
//ORIGINAL LINE: double[][] d = new double[20][3];
		std::vector<std::vector<double>> d = RectangularVectors::RectangularDoubleVector(20, 3);
		double s0 = 1 + xi;
		double t0 = 1 + et;
		double d0 = 1 + ze;
		double s1 = 1 - xi;
		...
		if (degeneration(ind) == 1)
		{
			std::vector<double> dn1(3);
			std::vector<double> dn2(3);
		...
		// Jacobian matrix ja
//JAVA TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] ja = new double[3][3];
		std::vector<std::vector<double>> ja = RectangularVectors::RectangularDoubleVector(3, 3);
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
		...
		double det = ja[0][0] * (ja[1][1] * ja[2][2] - ja[2][1] * ja[1][2]) - ja[1][0] * (ja[0][1] * ja[2][2] - ja[0][2] * ja[2][1]) + ja[2][0] * (ja[0][1] * ja[1][2] - ja[0][2] * ja[1][1]);
		if (det <= 0)
		{
            UTIL::errorMsg(L"Negative/zero Jacobian determinant for 20N element " + std::to_wstring(static_cast<float>(det)));
		}
		// Jacobian inverse ja1
//JAVA TO C++ CONVERTER NOTE: The following call to the 'RectangularVectors' helper class reproduces the rectangular array initialization that is automatic in Java:
//ORIGINAL LINE: double[][] ja1 = new double[3][3];
		std::vector<std::vector<double>> ja1 = RectangularVectors::RectangularDoubleVector(3, 3);
		double v = 1.0 / det;
	...
	void ShapeQuad3D::shapeDerivFace(double xi, double et, std::vector<int> &ind, std::vector<double> &an, std::vector<std::vector<double>> &dn)
	{
		double x1 = 1 - xi;
		double x2 = 1 + xi;
		double e1 = 1 - et;
		...
		dn[1][0] = dn[1][1] = dn[3][0] = dn[3][1] = dn[5][0] = dn[5][1] = dn[7][0] = dn[7][1] = 0;
